...
for (ith in 1:10){
  ...
  rf <- randomForest(response_points ~ ., data=train_rf, ntree=num_tree,nodesize=as.integer(nrow(train_rf)*minimum_nodes))

  mse.rf <- mean((test_rf$response_points - predict(rf, test_rf[,columns_rf]))^2)

  print(mse.rf)
}

print(importance(rf))
png(filename="feature_rf_1_50.png")
varImpPlot(rf,type=2,main='feature importance')
dev.off()
